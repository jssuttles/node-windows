...
var daemon = function(config){
  ...
  Object.defineProperties(this,{
    ...
    _directory: {
      ...
      writable: true,
      configurable: false,
      value: config.dir || (config.script !== null ? path.dirname(config.script) : null)
    },
    ...
    root: {
      enumerable: true,
      get: function(){ return this.directory(config.dir);}
    },
   ...
   */
    ...
    install: {
      ...
      value: function(dir){
        ...
        if (this.exists){
          ...
          if (!fs.existsSync(path.join(this.root,this._configFile))) {
            this.log.warn('The .NET config file is missing or cannot be found ('+path.join(this.root,this._configFile)+')');
            missing = true;
          }
        ...
        dir = this.directory(dir || config.dir);
        ...
        try {
          fs.exists(dir,function(exists){
            ...
            fs.writeFile(path.resolve(dir,me.id+'.xml'),me._xml,function(err){
              ...
              winsw.createConfigFile(me.id, dir, function(err) {
                if (err) {
                  throw err;
                }
                // Write the exe file
          ...
          me.execute('"'+path.resolve(me.root,me._exe)+'" uninstall',function(error,stdout,stderr){
            ...
            if (error){
              ...
              setTimeout(function() {
                ...
                if (uninstallError) {
                  setTimeout(function() {
                    if (fs.existsSync(path.join(me.root,me.id + '.exe')) || fs.existsSync(path.join(me.root,me.id + '.xml'))) {
                      return me.emit('error', 'exe and/or xml still exist');
                    }
                    me.emit('uninstall');
                  }, 1500);
                } else {
    ...
    isRunning: {
      ...
      value: function(callback) {
        if (this.exists) {
          exec('"'+path.resolve(this.root,this._exe)+'" status', function(err, stdout, stderr){
            callback(stdout.toString().includes('Started'));
          });
        } else {
          callback(false);
        }
