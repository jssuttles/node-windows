module.exports = {
   ...
   *
   * - *execPath* The path of the executable to run (usually leave blank and let it be node.exe)
   * - *arguments* If execPath is specified that's not node.exe, include arguments here instead of in flags or scripts
  ...
  generateXml: function(config){
    ...
    if (config.execPath.indexOf('node.exe') === -1) { // if not node.exe we don't understand arguments, and should let the user figure it out
      config.flags = '';
      config.script = '';
    }
  ...
  createConfigFile: function(name, dir, callback) {
    var fs = require('fs'), p = require('path');
    ...
    var origin = p.join(__dirname,'..','bin','winsw','winsw.exe.config'),
        dest = p.join(dir,name.replace(/[^\w]/gi,'').toLowerCase()+'.exe.config');
        
    var cbCalled = false;

    var rd = fs.createReadStream(origin);
